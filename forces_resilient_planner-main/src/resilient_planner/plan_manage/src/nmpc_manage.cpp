...
void NMPCManage::init(ros::NodeHandle &nh)
{
  ...
  int sim_odom_type;
  nh.param("nmpc/sim_odom_type", sim_odom_type, 1);  /// 里程计类型：0转置 和 1不转置
...
{
  if (!exec_mpc_)   /// execFSMCallback里面根据kino搜索成功与否，进行的exec_mpc_变量和exec_state_进行更新
      return;

  int status = nmpc_solver_.solveNMPC(stateOdom_, external_acc_);  /// nmpc求解的状态对 exec_mpc_ exec_state_进行更新
  //  1 --- success
  //  0 --- at global end
...
void NMPCManage::execFSMCallback(const ros::TimerEvent &e)
{
  ...
  switch (exec_state_)
  {
    ...
    case WAIT_TARGET:
    {
      ...
      { 
        changeFSMExecState(INIT_YAW, "FSM");   /// 初始化偏航角
        call_init_yaw_ = true;
      }
      break;
    }
    ...
    case INIT_YAW:
    {
      if (call_init_yaw_)
      {
        ...
        init_yaw_ = atan2(dir(1), dir(0));    /// 朝向目标运动

        if (abs(stateOdom_(8) - init_yaw_) >= 0.8)   /// 当前的航向和初始化的航向偏差大于0.8
        {
          nmpc_solver_.callInitYaw(stateOdom_, init_yaw_);  ///  定义init_yaw_dot_的数值和cmd_status_变量的数值
        }
        call_init_yaw_ = false;
        break;   /// 直接跳出该switch  先规划偏航角，调整好偏航角后，再规划无人机的轨迹
      }
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      if (plan_fail_count_ > 3){      /// 规划失败的次数大于3  重新设置目标点
        have_target_ = false;
        changeFSMExecState(WAIT_TARGET, "FSM");
        plan_fail_count_ = 0;
        break;
      ...
      if (nmpc_solver_.getKinoPath(stateOdom_, end_pt_, external_acc_, false))   /// 离散运动初始点
      {
        ...
        nmpc_solver_.getKinoTraj(kino_path_); ///  kino_path_ = kino_path_
    ...
    case REPLAN_TRAJ:
    {
      ...
      if (nmpc_solver_.getKinoPath(stateOdom_, end_pt_, external_acc_, true)){   /// 重新kino路径搜索
...
void NMPCManage::extforceCallback(const geometry_msgs::WrenchStamped &msg)
{
  ...
  if (consider_force_)
  {
    double diverse = max(max(abs(msg.wrench.force.x), abs(msg.wrench.force.y)), abs(msg.wrench.force.z)); /// max值
    if (diverse <= ext_noise_bound_)  /// 判断是否是噪声
    {
    ...
    else
    {
      ...
      if (surpass > ext_noise_bound_)  /// 上一次和当前的外力超过的噪声的界限   变化太大，需要重规划
      { 
        ...
        if (surpass_count_ >= 1)
        {
          ...
          if ( have_target_){
            ...
            changeFSMExecState(REPLAN_TRAJ, "SAFETY");  /// 外力变化太大，重规划轨迹
          }
          ...
          if (surpass > 10.0)
          {
            ...
            have_target_ = false;
            changeFSMExecState(WAIT_TARGET, "SAFETY");  /// 外力太激烈，等待新目标
          }
        }
      }
      else
...
void NMPCManage::odometryCallback(const nav_msgs::Odometry &msg)
{
  ...
  mav_msgs::getEulerAnglesFromQuaternion(quaternion, &temp2);  /// 获得实时姿态角
  stateOdom_.segment(6, 3) = temp2;   /// 获取从向量的第i个元素开始的n个元素：vector.segment(i,n)
...
void NMPCManage::odometryTransCallback(const nav_msgs::Odometry &msg)
{
  ...
  const Eigen::Matrix3d R_W_I = odometry.orientation_W_B.toRotationMatrix();  /// R_W_I: 从机体系到惯性系
