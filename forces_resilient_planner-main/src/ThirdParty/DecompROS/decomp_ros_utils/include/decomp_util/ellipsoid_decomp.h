...
public:
  ...
  * @param path The path to dilate（扩张，膨胀）
...
protected:
 ...
 vec_E<Ellipsoid<Dim>> ellipsoids_;     /// ellipsoid.h
 vec_E<Polyhedron<Dim>> polyhedrons_;   /// polyhedron.h
 std::vector<std::shared_ptr<LineSegment<Dim>>> lines_;  /// 存放指针（LineSegment类）的向量
